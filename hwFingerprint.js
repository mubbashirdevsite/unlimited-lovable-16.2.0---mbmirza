...
function qlNormalizeFingerprint(value) {
  var str = String(value || "").trim();
  if (!str || str === "undefined" || str === "null" || str === "[object Object]") return "";
  return str.slice(0, 200);
}

function qlRandomFingerprint() {
  try {
...
function qlSaveFingerprint(fp) {
  var clean = qlNormalizeFingerprint(fp) || qlRandomFingerprint();
  _cachedFingerprint = clean;
  try {
    chrome.storage.local.set({
      ql_hw_fingerprint: clean,
      ql_device_id: clean,
      ql_trial_device_id: clean
    });
  } catch(e) {}
  return clean;
}
...
async function getHardwareFingerprint() {
  ...
  return new Promise(async (resolve) => {
    chrome.storage.local.get(["ql_hw_fingerprint", "ql_device_id", "ql_trial_device_id"], async (res) => {
      var stored = qlNormalizeFingerprint(res && (res.ql_device_id || res.ql_hw_fingerprint || res.ql_trial_device_id));
      if (stored) {
        resolve(qlSaveFingerprint(stored));
      } else {
        try {
          const fp = await generateHardwareFingerprint();
          resolve(qlSaveFingerprint(fp));
        } catch(e) {
          // Fallback to random UUID if fingerprint fails completely
          resolve(qlSaveFingerprint(qlRandomFingerprint()));
        }
      }
    });
  });
