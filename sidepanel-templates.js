// ============================================================
// lovax - Side Panel Templates v2.0
// Visual shell only — keep element IDs for business logic.
// ============================================================
...
function spGateShell(innerHtml) {
  return '<div class="sp-license-gate">' + innerHtml + '</div>';
}

function spTemplateLicenseGate() {
  return spGateShell(
    '<div class="sp-lock-icon" aria-hidden="true">🔐</div>' +
    ...
    '<div class="sp-trial-divider"><span>or</span></div>' +
    '<button class="sp-btn-secondary sp-btn-trial" id="sp-trial-btn" type="button">🎁 Start Free Trial</button>' +
    '<p class="sp-gate-hint">No sign-up required · one trial per device</p>' +
    '<div class="sp-log" id="sp-license-log"></div>' +
    '<div class="sp-log" id="sp-trial-log" style="margin-top:8px"></div>'
  );
}

function spTemplateAuthGate() {
  return '<div id="sp-update-banner" style="display:none;margin-bottom:14px"></div>' +
    spGateShell(
      '<div class="sp-lock-icon" aria-hidden="true">✦</div>' +
      '<p class="sp-gate-title">Sign in</p>' +
      '<p class="sp-gate-desc">Sign in with your account or use a license key.</p>' +
      '<div class="sp-auth-tabs">' +
        '<button class="sp-auth-tab sp-auth-tab-active" data-auth-tab="account">Account</button>' +
        '<button class="sp-auth-tab" data-auth-tab="license">License key</button>' +
      '</div>' +
      '<div class="sp-auth-pane" data-auth-pane="account">' +
        '<button class="sp-btn-google" id="sp-auth-google-btn" type="button">' +
        ...
        '<input class="sp-input" id="sp-auth-email" type="email" placeholder="Email" autocomplete="email" spellcheck="false">' +
        '<input class="sp-input" id="sp-auth-pass" type="password" placeholder="Password" autocomplete="current-password">' +
        '<button class="sp-btn-primary" id="sp-auth-login-btn">Sign in</button>' +
        '<div class="sp-log" id="sp-auth-log"></div>' +
      '</div>' +
      '<div class="sp-auth-pane" data-auth-pane="license" style="display:none">' +
        '<input class="sp-input" id="sp-license-input" placeholder="Your license key" spellcheck="false">' +
        '<button class="sp-btn-primary" id="sp-validate-btn">Validate License</button>' +
        '<div class="sp-log" id="sp-license-log"></div>' +
      '</div>' +
      '<div class="sp-trial-divider"><span>or try it free</span></div>' +
      '<button class="sp-btn-secondary sp-btn-trial" id="sp-trial-btn" type="button">🎁 Start Free Trial</button>' +
      '<p class="sp-gate-hint">Instant access · no sign-up · one trial per device</p>' +
      '<div class="sp-log" id="sp-trial-log" style="margin-top:8px"></div>'
    );
}

function spTemplateLicensePicker(licenses, userEmail) {
  var rows = '';
  for (var i = 0; i < licenses.length; i++) {
    var l = licenses[i];
    var planName = spEscapeHtml(l.plan_name || l.plan_code || 'License');
    var expires = l.expires_at ? new Date(l.expires_at).toLocaleDateString() : '—';
    var status = spEscapeHtml(l.status || '');
    var keyMasked = spEscapeHtml((l.key || '').slice(0, 4) + '••••' + (l.key || '').slice(-4));
    var disabled = (l.status && l.status !== 'active') ? ' disabled' : '';
    rows += '<div class="sp-lic-card">' +
      '<div class="sp-lic-plan">' + planName + '</div>' +
      '<div class="sp-lic-meta">Key: <code>' + keyMasked + '</code></div>' +
      '<div class="sp-lic-meta">Status: ' + status + ' • Expires: ' + spEscapeHtml(expires) + '</div>' +
      '<button class="sp-btn-primary sp-lic-use" data-lic-key="' + spEscapeHtml(l.key) + '"' + disabled + '>Use this license</button>' +
    '</div>';
  }
  if (!licenses.length) {
    rows = '<p class="sp-gate-desc">No active licenses on your account. Contact support.</p>';
  }
  return '<div id="sp-update-banner" style="display:none;margin-bottom:14px"></div>' +
    spGateShell(
      '<div class="sp-lock-icon" aria-hidden="true">🔑</div>' +
      '<p class="sp-gate-title">Choose a license</p>' +
      '<p class="sp-gate-desc">Signed in as <b>' + spEscapeHtml(userEmail || '') + '</b></p>' +
      '<div class="sp-lic-list">' + rows + '</div>' +
      '<button class="sp-btn-secondary" id="sp-lic-back">Sign out</button>' +
      '<div class="sp-log" id="sp-lic-log"></div>'
    );
}

function spGetSocialItems() {
  try {
    if (typeof window !== "undefined" && typeof window.EXT_SOCIAL_ITEMS === "function") {
      return window.EXT_SOCIAL_ITEMS() || [];
    }
  } catch (_) {}
  var get = function (name) {
    try {
      if (typeof window !== "undefined" && typeof window.EXT_LINK === "function") {
        return String(window.EXT_LINK(name) || "").trim();
      }
    } catch (_) {}
    return "";
  };
  ...
  try {
    var custom = (typeof window !== "undefined" && window.EXT_CUSTOM_LINKS) || [];
...
function spTemplateSocialLinks(opts) {
  opts = opts || {};
  var title = opts.title || "Community & support";
  var items = spGetSocialItems();
  var buttons = "";
  for (var i = 0; i < items.length; i++) {
    var it = items[i];
    var url = spSanitizeUrl(it.url);
    if (!url) continue;
    var cls = it.custom ? "custom" : String(it.key).replace(/[^a-z0-9_-]/gi, "");
    buttons +=
      '<a class="sp-social-btn sp-social-btn-' + cls + '" href="' + url + '" data-ext-link="' + spEscapeHtml(it.key) + '" target="_blank" rel="noopener noreferrer">' +
        '<span class="sp-social-ico" aria-hidden="true">' + spEscapeHtml(it.icon) + '</span>' +
        '<span class="sp-social-label">' + spEscapeHtml(it.label) + '</span>' +
      '</a>';
  }
  if (!buttons) return "";
  return (
    '<div class="sp-social-card" style="width:100%;margin-top:12px">' +
      '<div class="sp-social-title">' + spEscapeHtml(title) + '</div>' +
      '<div class="sp-social-grid">' + buttons + '</div>' +
    '</div>'
  );
}

function spTemplateChannelSocialButtons() {
  var items = spGetSocialItems();
  var buttons = "";
  var joinLabels = {
    whatsapp: "Join WhatsApp Channel",
    youtube: "Join YouTube Channel & Unlock",
    official_channel: "Join Official Channel",
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var url = spSanitizeUrl(it.url);
    if (!url) continue;
    var platform = it.custom ? "custom" : String(it.key).replace(/[^a-z0-9_-]/gi, "");
    var label = it.custom
      ? (it.icon + " " + it.label)
      : (it.icon + " " + (joinLabels[it.key] || it.label));
    var i18nAttr = it.i18n ? ' data-i18n="' + spEscapeHtml(it.i18n) + '"' : "";
    buttons +=
      '<a class="sp-btn-primary sp-community-btn sp-community-btn-' + platform + ' sp-channel-social" href="' + url +
      '" data-ext-link="' + spEscapeHtml(it.key) + '" data-channel-unlock="1" target="_blank" rel="noopener noreferrer"' + i18nAttr + '>' +
      spEscapeHtml(label) +
      '</a>';
  }
  return buttons;
...
function spTemplateChannelGate() {
  return '<div id="sp-update-banner" style="display:none;margin-bottom:14px"></div>' +
    spGateShell(
      '<div class="sp-lock-icon" style="animation:sp-pulse 2s infinite" aria-hidden="true">🔥</div>' +
      '<p class="sp-gate-title" data-i18n="extension.channel_gate_title">Unlock Extension</p>' +
      '<p class="sp-gate-desc" data-i18n="extension.channel_gate_desc">Join our channels to get updates, tips, and access information instantly.</p>' +
      '<div class="sp-channel-social-list" style="width:100%;display:flex;flex-direction:column;gap:10px">' +
        spTemplateChannelSocialButtons() +
      '</div>' +
      '<button class="sp-btn-secondary sp-channel-signin-btn" id="sp-channel-signin-btn" type="button" data-i18n="extension.already_have_account">🔑 I have an account / license — Sign in</button>' +
      '<div class="sp-log" id="sp-channel-log"></div>'
    );
}

function spTemplateAccountPage(info) {
  info = info || {};
  var email = spEscapeHtml(info.email || '—');
  var name = spEscapeHtml(info.name || '—');
  var key = info.key || '';
  var keyMasked = key ? spEscapeHtml(key.slice(0, 4) + '••••' + key.slice(-4)) : '—';
  var status = spEscapeHtml(info.status || '—');
  var expires = info.expiresAt ? spEscapeHtml(new Date(info.expiresAt).toLocaleString()) : '—';
  var activated = info.activatedAt ? spEscapeHtml(new Date(info.activatedAt).toLocaleString()) : '—';
  var statusBadge = spTemplateStatusBadge(info.status);
  return spGateShell(
    '<div class="sp-lock-icon" aria-hidden="true">👤</div>' +
    '<p class="sp-gate-title">My Account</p>' +
    '<div class="sp-profile-card" style="width:100%;margin-top:4px;text-align:left">' +
      '<div class="sp-profile-top"><span class="sp-profile-name">' + name + '</span>' + statusBadge + '</div>' +
      '<div class="sp-lic-meta">Email: <b>' + email + '</b></div>' +
      '<div class="sp-lic-meta">License: <code>' + keyMasked + '</code></div>' +
      '<div class="sp-lic-meta">Status: ' + status + '</div>' +
      '<div class="sp-lic-meta">Activated: ' + activated + '</div>' +
      '<div class="sp-lic-meta">Expires: ' + expires + '</div>' +
    '</div>' +
    '<div class="sp-lic-card" style="width:100%;margin-top:10px">' +
      '<div class="sp-lic-meta" style="margin-bottom:6px">Change license key</div>' +
      '<input class="sp-input" id="sp-account-new-lic" placeholder="New license key" spellcheck="false">' +
      '<button class="sp-btn-primary" id="sp-account-change-lic" style="margin-top:8px">🔑 Replace license</button>' +
    '</div>' +
    spTemplateSocialLinks({ title: "Community & support" }) +
    '<button class="sp-btn-primary" id="sp-account-back" style="margin-top:12px">← Back</button>' +
    '<button class="sp-btn-secondary" id="sp-account-logout" style="margin-top:8px">Sign out</button>' +
    '<div class="sp-log" id="sp-account-log"></div>'
  );
}

function spTemplateNoLicenseAccount(info, pricingUrl) {
  info = info || {};
  var email = spEscapeHtml(info.email || '—');
  var name = spEscapeHtml(info.name || info.email || '—');
  var userId = spEscapeHtml(info.userId || '—');
  var via = spEscapeHtml(info.via || 'account');
  var safePricing = spSanitizeUrl(pricingUrl) || '#';
  return spGateShell(
    '<div class="sp-lock-icon" aria-hidden="true">👤</div>' +
    '<p class="sp-gate-title">Signed in — no active license</p>' +
    '<div class="sp-profile-card" style="width:100%;margin-top:4px;text-align:left">' +
      '<div class="sp-profile-top"><span class="sp-profile-name">' + name + '</span>' +
        '<span class="sp-status-badge sp-badge-test">No license</span></div>' +
      '<div class="sp-lic-meta">Email: <b>' + email + '</b></div>' +
      '<div class="sp-lic-meta">User ID: <code>' + userId + '</code></div>' +
      '<div class="sp-lic-meta">Signed in via: ' + via + '</div>' +
    '</div>' +
    '<p class="sp-gate-desc" style="margin-top:14px">If you already have a license key, enter it below. Otherwise choose a plan on our website.</p>' +
    '<input class="sp-input" id="sp-nolic-key" placeholder="Your license key" spellcheck="false">' +
    '<button class="sp-btn-primary" id="sp-nolic-activate">Activate license</button>' +
    '<a class="sp-btn-primary sp-community-btn" href="' + safePricing + '" target="_blank" rel="noopener noreferrer" style="margin-top:10px;display:block;text-align:center;text-decoration:none">🛒 Choose a plan</a>' +
    spTemplateSocialLinks({ title: "Community & support" }) +
    '<button class="sp-btn-secondary" id="sp-nolic-logout" style="margin-top:8px">Sign out</button>' +
    '<div class="sp-log" id="sp-license-log"></div>'
  );
}
...
function spTemplateAlert(title, message, opts) {
  opts = opts || {};
  var icon = opts.icon || (opts.error ? '❌' : '✅');
  return '<div class="sp-alert-box' + (opts.error ? ' sp-alert-box-error' : '') + '">' +
    '<div class="sp-alert-icon">' + icon + '</div>' +
...
function spTemplateUpdateBanner(version, changelog, dlUrl, isMandatory) {
  var badge = isMandatory
    ? '<span class="pk-update-banner-badge pk-update-banner-badge--req">Required</span>'
    : '<span class="pk-update-banner-badge">Optional</span>';
  var icon = isMandatory ? '⚠️' : '🔔';
  return '<div class="pk-update-banner' + (isMandatory ? ' pk-update-banner--mandatory' : '') + '">' +
...
function spTemplateMandatoryOverlay(version, changelog, dlUrl) {
  return '<div class="pk-update-overlay" id="sp-mandatory-update" role="dialog" aria-modal="true" aria-labelledby="sp-mandatory-title" tabindex="-1">' +
    '<div class="pk-update-overlay-card">' +
      '<div class="pk-update-overlay-icon">⚠️</div>' +
      '<h2 class="pk-update-overlay-title" id="sp-mandatory-title">Update required — v' + version + '</h2>' +
      '<p class="pk-update-overlay-text">' + (changelog || 'A new mandatory version is available. Please install it to continue using the extension.') + '</p>' +
      (dlUrl ? '<a href="' + dlUrl + '" target="_blank" rel="noopener noreferrer" class="pk-update-overlay-btn">Download v' + version + '</a>' : '') +
      '<p class="pk-update-overlay-hint">You cannot continue before installing this required update. After downloading, open <code>chrome://extensions</code>, remove the old version and load the new one.</p>' +
    '</div>' +
  '</div>';
}
