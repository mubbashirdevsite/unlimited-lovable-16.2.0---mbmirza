// ============================================
// lovax – Business Logic (content)
// HTML templates are in content-templates.js
// ============================================

// Mobile-safe startup: never clear extension storage or destroy the host page.
// Older builds had an integrity timer here that could false-trigger on Android
// Chromium browsers and make the extension look like it deleted itself.

console.log("[ContentScript] lovax loaded");
...
const DISCORD_URL = (typeof window !== "undefined" && window.EXT_LINK)
  ? (window.EXT_LINK("discord") || window.EXT_LINK("official_channel") || "") : "";
...
const VERSIONS_URL_POPUP = API_BASE + "/rest/v1/extension_versions?select=version,changelog,file_path,is_current,is_mandatory&order=created_at.desc&limit=1&is_current=eq.true";
...
const CURRENT_EXT_VERSION = typeof extensionVersionShort === "function"
...
function setPkCreditBypass(_on) {
  // Always ON — old devices previously turned this off when license verify
  // failed, which left stale MAIN-world hooks without fix_error rewrite.
  // New + old devices must keep the same always-on path.
  if (typeof window.__pkSetCreditBypass === "function") {
    window.__pkSetCreditBypass(true);
    return;
  }
  try {
    localStorage.setItem("__ql_bypass_active", "1");
    document.documentElement.setAttribute("data-ql-bypass", "1");
    window.postMessage({ type: "qlNormalSend", active: false }, "*");
    window.postMessage({ type: "qlBypassState", active: true }, "*");
  } catch (e) {}
}

function activateBypass() { setPkCreditBypass(true); }
function deactivateBypass() { setPkCreditBypass(true); }
...
function getDeviceId(){
  return getHardwareFingerprint().then(function(id) {
    var clean = String(id || '').trim();
    if (clean && clean !== 'undefined' && clean !== 'null') {
      try { chrome.storage.local.set({ ql_device_id: clean, ql_hw_fingerprint: clean, ql_trial_device_id: clean }); } catch(e) {}
    }
    return clean;
  });
}
...
function qlIsMobileInlineRuntime() {
  try {
    var ua = (navigator.userAgent || "").toLowerCase();
    var uaMobile = !!(navigator.userAgentData && navigator.userAgentData.mobile === true);
    return uaMobile || /android|mobile|kiwi|yabrowser|crios|edga\//i.test(ua);
  } catch (_) {
    return false;
  }
}
  ...
  chrome.storage.local.get(["ql_channel_redirected", "ql_license_valid","ql_license_key","ql_minimized","ql_height","ql_dark_mode","ql_user_name","ql_expires_at","ql_activated_at","ql_license_status","ql_validity_minutes","ql_session_id"], async (res) => {
    ...
    if (INTERNAL_LICENSE_MODE) {
      try {
        ...
        console.warn("[QL] Test mode session setup failed", e);
      }
      showMainUI(box);
      setPkCreditBypass(true);
      setupDrag();
    ...
    if (!res.ql_channel_redirected) {
      ...
      if(INTERNAL_LICENSE_MODE && !res.ql_license_valid) {
        ...
        // SERVER-AUTHORITATIVE: never trust ql_license_valid alone.
        // Ask the license server before rendering the main UI.
        if (typeof pkAssertServerLicense !== "function") {
          // Guard file missing/tampered → refuse to unlock the UI.
          console.warn("[QL] License guard missing — locking UI");
          showLicenseGate(box);
          return;
        }
        try {
          var _chk = await pkAssertServerLicense({ force: true });
          if (!_chk || !_chk.ok) {
            qlHandleLicenseInvalid({
              reason: (_chk && _chk.data && _chk.data.reason) || "invalid",
              message: (_chk && _chk.data && _chk.data.message) || "License not active."
            });
            return;
          }
          var _d = _chk.data || {};
          qlUserName = normalizeLicenseUserName(_d.user_name || res.ql_user_name);
          qlApplyLicenseApiData(_d);
          qlSessionId = _d.session_id || res.ql_session_id || null;
          chrome.storage.local.set(Object.assign(
            { ql_user_name: qlUserName, ql_session_id: qlSessionId },
            typeof pkLicenseStoragePatch === "function" ? pkLicenseStoragePatch(_d) : {}
          ));
        } catch (e) {
          console.warn("[QL] License verify failed", e);
          showLicenseGate(box);
          return;
        }
        showMainUI(box);
  ...
  setTimeout(() => {
    ...
    if(btn) {
      btn.addEventListener("click", () => {
        chrome.runtime.sendMessage({ action: "openTab", url: (window.EXT_LINK ? (window.EXT_LINK("youtube") || window.EXT_LINK("official_channel") || "") : "") });
...
async function validateLicense(){
  ...
  if(!key){
    if(log){ log.className = "ql-log-error"; log.innerText = "⚠ " + qlUserText("Enter a key"); }
    return;
  }
  ...
  try{
    ...
    const data = await bgFetch(VALIDATE_URL, {
      method: "POST",
      headers: apiHeaders({ "Content-Type": "application/json" }),
        body: JSON.stringify({ license_key: key, device_id: qlDeviceId, device_name: (navigator && navigator.userAgent) || "", platform: (navigator && navigator.platform) || "", extension_version: CURRENT_EXT_VERSION })
    });
...
// IDs of buttons that require an active, server-verified license before firing.
var QL_GUARDED_BUTTON_IDS = [
  "ql-send-btn",
  "ql-optimize-btn",
  "ql-native-chat-btn",
  "ql-native-chat-toggle",
  "ql-watermark-btn",
  "ql-remove-watermark",
  "ql-shield-btn",
  "ql-download-project",
  "ql-download-project-btn",
  "ql-quick-init",
  "ql-create-project-btn",
  "ql-publish-project",
  "ql-publish-project-btn",
  "ql-enable-cloud",
  "ql-enable-cloud-btn",
  "ql-file-attach-btn",
  "ql-speech-btn"
...
var _qlActionGuardInstalled = false;
function qlInstallActionGuard(box) {
  if (_qlActionGuardInstalled) return;
  _qlActionGuardInstalled = true;

  // Capture-phase listener so we always run BEFORE any per-button handler.
  var handler = function(ev) {
    var t = ev.target;
    if (!t || !t.closest) return;
    var btn = null;
    for (var i = 0; i < QL_GUARDED_BUTTON_IDS.length; i++) {
      var found = t.closest("#" + QL_GUARDED_BUTTON_IDS[i]);
      if (found) { btn = found; break; }
    }
    if (!btn) return;
    if (btn.dataset.qlLicenseOk === "1") {
      // Cleared by a recent server check; let the real handler run.
      btn.dataset.qlLicenseOk = "";
      return;
    }
    // Block the click and verify with the server first.
    ev.stopImmediatePropagation();
    ev.preventDefault();
    if (INTERNAL_LICENSE_MODE) {
      btn.dataset.qlLicenseOk = "1";
      btn.click();
      return;
    }
    if (typeof pkAssertServerLicense !== "function") {
      showCustomAlert("Access Denied", "License guard is missing. Reinstall the extension.");
      return;
    }
    ...
    pkAssertServerLicense({ force: false }).then(function(r) {
      ...
      btn.dataset.qlLicenseOk = "1";
      // Re-fire the click so the real handler runs.
      btn.click();
    }).catch(function(err) {
      try { btn.disabled = prevDisabled; } catch(e){}
      showCustomAlert("Connection Error", (err && err.message) || "Could not reach the license server.");
    });
  };
  var root = (box && box.ownerDocument) || document;
...
function setupOptimize(){
  ...
  btn.addEventListener("click", async () => {
    ...
    function enhanceLocal(p) {
      if (/^Goal:/im.test(p) || /^Request:/im.test(p) || /^الطلب:/m.test(p)) return p;
      if (/[\u0600-\u06FF]/.test(p)) {
        return "نفّذ الطلب التالي بعناية وبشكل كامل.\n\nالطلب:\n" + p +
          "\n\nإرشادات:\n- حافظ على أسلوب التصميم الحالي\n- اجعل الواجهة متجاوبة ومصقولة\n- لا تحذف ميزات غير مرتبطة";
      }
      return "Please implement the following request carefully and completely.\n\nRequest:\n" + p +
        "\n\nGuidelines:\n- Keep existing design language unless asked to change it\n- Prefer clear, production-ready code\n- Make UI responsive and polished";
    }

    try {
      let enhanced = enhanceLocal(original);
      if (licenseKey) {
        try {
      ...
      // Only update extension chat — never send to Lovable from Optimize
    ...
    try {
      ...
      qlSpeechRecognition.onerror = (event) => {
        ...
        if(event.error === "not-allowed") {
          ...
          showCustomAlert("Voice Error", qlUserText("Error: ") + event.error);
        }
      };
...
function setupWatermarkButton(){
  ...
  btn.addEventListener("click", async function(){
    ...
    try {
      ...
      if(log){ log.className = "ql-log-error"; log.innerText = "✗ " + qlUserText(err.message || err); }
    } finally {
      btn.disabled = false;
...
function setupPublishProject(){
  ...
  btn.addEventListener("click", async function(){
    ...
    if(!projectId || !token){
      if(log){ log.className = "ql-log-error"; log.innerText = "⚠ " + qlUserText("Project not synced."); }
      btn.disabled = false;
...
function qlHandleLicenseInvalid(data) {
  ...
  // Old-device orphan sessions: storage points at deleted DB rows.
  var maybeReset =
  ...
  maybeReset.then(function (res) {
    if (res && res.reset) {
      if (typeof pkInvalidateAssertCache === "function") pkInvalidateAssertCache();
      // Keep pageHook always-on rewrite; content-bridge ignores deactivate.
...
function injectShieldOverlay(){
  ...
  overlay.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    ...
    '</svg>' +
    '<span class="ql-shield-overlay-text">🛡️ Protected by Mirza-Lovable</span>' +
...
function startHeartbeat(licenseKey){
  ...
  qlHeartbeatInterval = setInterval(async () => {
    try {
      const data = await bgFetch(VERIFY_LICENSE_URL, {
        method: "POST",
        headers: apiHeaders({ "Content-Type": "application/json" }),
        body: JSON.stringify({ license_key: licenseKey, session_id: qlSessionId, session_token: qlSessionId, heartbeat: true, device_id: qlDeviceId, extension_version: CURRENT_EXT_VERSION })
      });
...
async function showPaymentUI(box, preselectedPkg){
  ...
  document.querySelectorAll(".ql-brl-buy").forEach(function(btn){
    btn.addEventListener("click", function(){
      ...
      if(!plan) return;
      var msg = "Hello! 👋 I am interested in the *" + plan.name + "* plan for Mirza-Lovable (R$ " + plan.price + " - " + plan.period + ").\n\nOpen channel for more information.";
      var url = DISCORD_URL;
  ...
  chrome.runtime.sendMessage({
    ...
    chrome.storage.local.get(["lovable_projectId","lovable_token"], (res)=>{
      ...
      if(res.lovable_projectId && token && isTokenFresh(token)){
        ...
        status.innerHTML = '<span class="ql-sync-text">⚠ ' + qlUserText('Log in on lovable.dev') + '</span>';
      } else {
        status.className = "ql-sync-status ql-sync-waiting";
...
function setupSend(){
  ...
  btn.addEventListener("click", async ()=>{
    ...
    if(!mensagem){
      if(log){ log.className = "ql-log-error"; log.innerText = "⚠ " + qlUserText("Empty prompt"); }
      return;
    }
    ...
    if (!projectId) {
      if (log) { log.className = "ql-log-error"; log.innerText = "⚠ " + qlUserText("Open lovable.dev on your project and wait for sync."); }
      return;
    }

    // Development/test mode: allow prompts without license enforcement so all users
    // can continue testing while the extension is being modified.
...
// ===== UPDATE CHECK (Popup) — reads info populated by background poller =====
async function checkForUpdatePopup() {
  try {
    var st = await new Promise(function(r){ chrome.storage.local.get(['ql_update_info'], r); });
    var upd = st && st.ql_update_info;
    if (!upd || !upd.available) return;
    var banner = document.getElementById('ql-update-banner');
    if (banner) {
      banner.innerHTML = qlTemplateUpdateBanner(upd.version, upd.changelog || '', upd.download_url || null, !!upd.is_mandatory);
      banner.style.display = 'block';
    }
    if (upd.is_mandatory) showMandatoryUpdateOverlay(upd);
  } catch(e) {}
}

function showMandatoryUpdateOverlay(upd) {
  try {
    if (document.getElementById('ql-mandatory-update')) return;
    var wrap = document.createElement('div');
    wrap.innerHTML = qlTemplateMandatoryOverlay(upd.version, upd.changelog || '', upd.download_url || null);
...
try {
  chrome.storage.onChanged.addListener(function(ch, area){
    if (area === 'local' && ch.ql_update_info) checkForUpdatePopup();
  });
} catch(e) {}
...
function injectNativeChatOverlay() {
  ...
  if (!document.getElementById("ql-native-badge")) {
    ...
    badge.innerHTML = "⚡ <span>Mirza-Lovable</span>";
...
(function initLovableAuthSync(){
  ...
  function run() {
    if (document.hidden) return; // skip when tab is in background — huge mobile battery/CPU saver
    chrome.runtime.sendMessage({
      action: "syncLovableAuth",
      tabUrl: location.href,
      projectId: projectIdFromPage()
  ...
  // Mobile devices poll far less often to avoid draining CPU/battery.
  var uaMobile = (navigator.userAgentData && navigator.userAgentData.mobile === true)
    || /android|mobile|kiwi|yabrowser|crios/i.test((navigator.userAgent || "").toLowerCase());
  var pollMs = uaMobile ? 30000 : 8000;
  run();
